...
const validPathDomains2 = /^https?:\/\/(invidio\.us\/|(www\.)?invidio.us\/(embed|v)\/)/;
exports.getURLVideoID = link => {
  const parsed = url.parse(link, true);
  let id = parsed.query.v;
  if ((validPathDomains.test(link) || validPathDomains2.test(link)) && !id) {
...
exports.stripHTML = html => html
  ...
  .replace(/<a\s+(?:[^>]*?\s+)?href=(?:["'])((?:https?|\/).*?)(?:['"]).*?<\/a>/gi,
    (_, p1) => url.resolve('https://invidio.us/', p1))
  .replace(/<.*?>/gi, '')
  .trim();
